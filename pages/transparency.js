  ...
  return (
    <div style={{ minHeight: "100vh", background: surface, color: ink, fontFamily: fontSans, fontSize: "13px", lineHeight: 1.6, letterSpacing: "-0.011em", WebkitFontSmoothing: "antialiased" }}>
      ...
      <div className="page-content" style={{ maxWidth: 780, margin: "0 auto", padding: "4.5rem 2rem 5rem" }}>
        ...
        {/* ── Aggregate stats ──────────────────────────────────────────── */}
        {/* Split deliberately. A single blended number would count records
            generated by our own automated security testing as if they were
            real activity — including a chain we broke on purpose. */}
        <div style={{ display: "flex", gap: "12px", flexWrap: "wrap", marginBottom: "12px" }}>
          <StatCard
            label="Decisions — development use"
            value={statsError ? "—" : stats ? stats.development_usage.decisions.toLocaleString() : "…"}
            sub={statsError ? "Couldn't reach AIDAL's servers just now." : `Across ${stats ? stats.development_usage.accounts : "—"} founder development, demo, and integration accounts. Not external customers.`}
          />
          <StatCard
            label="Decisions — internal testing"
            value={statsError ? "—" : stats ? stats.internal_testing.decisions.toLocaleString() : "…"}
            sub={`Generated by automated security and integration testing across ${stats ? stats.internal_testing.accounts : "—"} accounts. Retained immutably, excluded from the figure on the left.`}
          />
        </div>
        ...
        <div style={{
          ...
          <strong style={{ color: ink, fontWeight: 600 }}>0 external customers.</strong> Every account above —
          all {statsError ? "" : stats ? stats.totals_including_test_data.accounts : ""} of them, and all of the decision volume with
          them — is the founder&apos;s own usage. No company outside AIDAL has signed up yet.
          Said plainly here instead of letting the raw numbers imply otherwise.
        </div>

        {/* ── Internal test disclosure ─────────────────────────────────── */}
        {stats && stats.internal_testing.notes.length > 0 && (
          <div style={{ marginBottom: "3rem" }}>
            <SectionLabel style={{ marginBottom: "0.875rem" }}>What the internal-testing records are</SectionLabel>
            <p style={{ fontSize: "12.5px", color: inkMuted, marginBottom: "1rem", lineHeight: 1.7 }}>
              These accounts were created by automated security and integration testing, not by real use. Their
              records are <strong style={{ color: ink }}>not deleted</strong> — this is an append-only evidence
              store, and quietly removing records to improve a public number is the exact behaviour AIDAL exists
              to make detectable. They are labelled and reported separately instead.
            </p>
            <div style={{ background: surface, border: `1px solid ${line}`, borderRadius: radiusLg, boxShadow: shadowXs, overflow: "hidden" }}>
              {stats.internal_testing.notes.map((n, i) => (
                <div key={n.account} style={{ padding: "1rem 1.5rem", borderBottom: i !== stats.internal_testing.notes.length - 1 ? `1px solid ${line}` : "none" }}>
                  <div style={{ fontFamily: fontMono, fontSize: "12px", color: ink, marginBottom: "6px" }}>{n.account}</div>
                  <div style={{ fontSize: "12.5px", color: inkMuted, lineHeight: 1.7 }}>{n.note}</div>
                </div>
              ))}
            </div>
          </div>
